...
int main(int argc, const char *argv[]) {
  ...
  for (int i = 0; i < M; i++) {
    for (int j = 0; j < K; j++) {
      if (j < 2048) {
        AVec[i*K + j] = 5;  // Correct row-major indexing
      }
      else if (j == 2048) {  // Fixed comparison operator
        AVec[i*K + j] = 1;
      }
      else {
        AVec[i*K + j] = 0;
  ...
  for (int i = 0; i < K; i++) {
    for (int j = 0; j < N; j++) {
      if (i == 2048) {  // Fixed comparison operator
        BVec[i*N + j] = 3;
      }
      else if (i > 2048) {
        BVec[i*N + j] = 0;
  ...
  std::string output_file = "matrix_output.csv";
  if (argc > 1) {
    // Allow specifying output filename as command line argument
    std::string arg1 = argv[1];
    if (arg1.find("--output=") == 0) {
      output_file = arg1.substr(9);
    }
  }
  
  std::cout << "Writing output matrix to " << output_file << std::endl;
  std::ofstream outfile(output_file);
  if (!outfile.is_open()) {
    std::cerr << "Error: Could not open file for writing: " << output_file << std::endl;
  } else {
    // Write CSV format (one row per line)
    for (int i = 0; i < M; i++) {
      for (int j = 0; j < N; j++) {
        outfile << static_cast<float>(CVec[i * N + j]);
        if (j < N - 1) outfile << ",";
      }
      outfile << "\n";
    }
    outfile.close();
    std::cout << "Successfully wrote matrix to " << output_file << std::endl;
  }
