...
def my_matmul():
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            def sequence(A, B, C):
                A_offset = 0 * M_div_m_div_n_cores * m * K
                C_offset = 0 * M_div_m_div_n_cores * m
                
                # Set up input B DMA once - outside the loop
                npu_dma_memcpy_nd(
                    metadata=inB_fifo,
                    bd_id=2, 
                    mem=B,
                ...
                # Set up output C DMA once
                npu_dma_memcpy_nd(
                    metadata=outC_fifos[0],
                    bd_id=0,
                    mem=C,
                ...
                for j in range(M_div_m_div_n_cores):
                    # Set up input A DMA for this chunk
                    npu_dma_memcpy_nd(
                        metadata=memA_fifos[0],
                        bd_id=1, 
                        mem=A,
                        offsets=[0, 0, 0, A_offset + j * m_x_K],
                        sizes=[1, K_div_k, m, k],
                        strides=[0, k, K, 1],
                    )
                    
                    # Wait for input A DMA to complete for this chunk
                    dma_wait(memA_fifos[0])
                
                # Wait for all DMAs to complete before returning
                dma_wait(inB_fifo, *outC_fifos)
