...
def my_matmul(M, K, N):
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            def sequence(A, B, C):
                m_factor = 1
                for j in range(M_div_m_div_n_cores // m_factor):
                    npu_dma_memcpy_nd(
                        metadata=inB_fifo,
                        bd_id=2,
                        mem=B,
                        sizes=[m_factor, 1, 1, K],
                        strides=[0, 0, 0, 1],
                    )
                    for i in range(n_cores):
                        A_offset = i * M_div_m_div_n_cores * m * K + j * m_factor * m_x_K
                        C_offset = i * M_div_m_div_n_cores * m + j * m_factor * m
                        ...
                        npu_dma_memcpy_nd(
                            ...
                            sizes=[1, 1, 1, C_sz_div_n_cores // M_div_m_div_n_cores * m_factor],
                            strides=[0, 0, 0, 1],
                        )
                    dma_wait(*outC_fifos)
