...
int main(int argc, const char *argv[]) {
  ...
  if (!errors) {
      ...
      // // std::cout << "Loading new xclbin: " << vm["new_xclbin"].as<std::string>() << "\n";
      ...
      // std::cout << "Registering new xclbin took " << std::chrono::duration_cast<std::chrono::microseconds>(t4 - t3).count() << "us." << std::endl;
      


      auto kernel1 = xrt::kernel(context, kernelName);
      // std::cout << "instantiating kernel\n";
      auto bo_instr1 = xrt::bo(device, instr_v1.size() * sizeof(int),
      XCL_BO_FLAGS_CACHEABLE, kernel1.group_id(1));
      // std::cout << "Writing data into buffer objects.\n";
      auto bo_a1 =
          xrt::bo(device, A1_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel1.group_id(3));
          // std::cout << "Writing A into buffer objects.\n";
      auto bo_b1 =
          xrt::bo(device, B1_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel1.group_id(4));
          // std::cout << "Writing B into buffer objects.\n";
      auto bo_out1 =
          xrt::bo(device, OUT_SIZE, XRT_BO_FLAGS_HOST_ONLY, kernel1.group_id(5));
          // std::cout << "Writing C into buffer objects.\n";
      ...
      auto very_end = std::chrono::high_resolution_clock::now();
      auto duration = std::chrono::duration_cast<std::chrono::microseconds>(very_end - very_begin).count();
      auto second_duration = std::chrono::duration_cast<std::chrono::microseconds>(stop1 - start1).count();
      std::cout << "second matrix multiplication took " << second_duration << "us." << std::endl;
